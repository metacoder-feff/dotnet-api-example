...
+ Tests isolation
  + Every test has its own unique tmp database (in postgress)
  + Every test has its own unique tmp Key/Channel prefix (in redis)
- Testing performance
  - (TODO) Run tests in parallel 
  - Postgress stores data in RAM-disc (devcontainer)
  - Redis stores data in RAM-disc (devcontainer)
+ Example API tests
